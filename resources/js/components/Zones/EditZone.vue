        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="editModalTitle">Modifier zone</h5>
            ...
            <!-- /.card -->
                ...
                <div class="modal-footer">
                    ...
                    <button hidden id="submitEditZone" type="submit" class="btn btn-primary" data-dismiss="modal" @click="update"></button>
                </div>
            </div>
        </div>
    </div>
...
import { required} from 'vuelidate/lib/validators';
    export default {
      ...
      data: function(){
        ...
        methods: {
          update(){
              axios.patch('/zones/edit/' + this.zoneToEdit.id, {
                  designation: this.designation,
              })
              .then(response => this.$emit('zone-updated',response))
              .catch(error => console.log(error));
          },
          checkEditForm(){
            this.$v.$touch()
            if(!this.$v.$invalid){
              var submitForm = document.getElementById('submitEditZone');
              submitForm.click();
            }else{
